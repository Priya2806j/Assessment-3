 ...
 WITH ProjectGroups AS (
    SELECT Task_ID, 
           Start_Date, 
           End_Date,
           ROW_NUMBER() OVER (ORDER BY Start_Date) - ROW_NUMBER() OVER (PARTITION BY 
Start_Date ORDER BY Task_ID) AS grp
    FROM Projects
 )
 ...
 FROM ProjectGroups
 GROUP BY grp
 ORDER BY DATEDIFF(day, MIN(Start_Date), MAX(End_Date)), MIN(Start_Date);-- Task 2
 SELECT S1.Name
 FROM Students S1
 ...
 WITH ContestStats AS (
    SELECT C.contest_id, 
           ...
           COALESCE(SUM(S.total_accepted_submissions), 0) AS 
...
total_accepted_submissions != 0
 ...
 WHERE D1.rn = 1
 ORDER BY D1.submission_date;-- Task 6
...
Manhattan_Distance
 ...
 ORDER BY C.company_code;-- Task 11
 SELECT S1.Name
 FROM Students S1
 JOIN Friends F ON S1.ID = F.ID
 JOIN Packages P1 ON S1.ID = P1.ID
...
ErrorAmount
 FROM Actual, Miscalculated;-- Task 20
